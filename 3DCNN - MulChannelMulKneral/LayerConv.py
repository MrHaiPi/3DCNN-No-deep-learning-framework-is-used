...
class LayerConv:
    ...
    def __init__(self, size_of_kernel, step, padding, act_fun_name, name):
        """
        :param size_of_kernel: 核尺寸，内容为：[个数，深度，时间域长度，空间域高度，空间域宽度]
    ...
    def think(self, input_data, pre_layer_name):
        ...
        if 'pool' in pre_layer_name or 'conv' in pre_layer_name:
            ...
            self.z = np.zeros((input_data.shape[0],
                               self.weight.shape[0],
                               (input_data.shape[2] + 2 * self.padding[0] - self.weight.shape[2]) // self.step[0] + 1,
                               (input_data.shape[3] + 2 * self.padding[1] - self.weight.shape[3]) // self.step[1] + 1,
                               (input_data.shape[4] + 2 * self.padding[2] - self.weight.shape[4]) // self.step[2] + 1))
            ...
            for i in range(input_data.shape[0]):
                ...
                for j in range(self.weight.shape[0]):
                    # 每个深度
                    for k in range(self.weight.shape[1]):
                        # 计算本层的状态值
                        self.z[i, j] += ConvolutionFunction.three_dem_convolution(self.weight[j, k], self.step, self.padding, input_data[i, k]) + self.bias[j]
                        # 计算本层的激活值
            ...
            for i in range(self.delta.shape[0]):
                ...
                for j in range(self.delta.shape[1]):
                    ...
                    for k in range(next_layer_w.shape[0]):
                        self.delta[i, j] += ConvolutionFunction.three_dem_convolution(nex_layer_w_r180[k, j], next_layer_step, next_layer_padding, next_layer_delta[i, k]) * self.act_fun[1](self.a[i, j])
        ...
        if 'pool' in pre_layer_name or 'conv' in pre_layer_name:
            ...
            for i in range(pre_layer_a.shape[0]):
                ...
                for j in range(self.weight.shape[0]):
                    ...
                    for k in range(self.weight.shape[1]):
                        # 调整w参数
                        w_delta[j, k] += 1 / pre_layer_a.shape[0] * ConvolutionFunction.three_dem_convolution(self.delta[i, j], self.step, self.padding, pre_layer_a[i, k])
                        # 调整b参数
                        b_delta[j] +=  1 / pre_layer_a.shape[0] * np.sum(self.delta[i, j])
