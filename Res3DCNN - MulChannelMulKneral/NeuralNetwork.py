        ...
        for i in range(self.size_layer.__len__()):
            ...
            if 'conv1' in self.size_layer[i]:
                ...
                if 'a' in self.size_layer[i]:
                    conv_kneral = np.array([4,3,3,3,3])
                else:
                    conv_kneral = np.array([4,4,3,3,3])
                self.layers.append(LayerConv.LayerConv(conv_kneral, np.array([1, 1, 1]), np.array([1, 1, 1]), 'leaky_relu', self.size_layer[i]))
                ...
                conv_kneral = None
                if 'a' in self.size_layer[i]:
                    conv_kneral = np.array([8,4,3,3,3])
                else:
                    conv_kneral = np.array([8,8,3,3,3])
                self.layers.append(LayerConv.LayerConv(conv_kneral, np.array([1, 1, 1]), np.array([1, 1, 1]), 'leaky_relu', self.size_layer[i]))
                self.layers_pool.append(None)
                # 更新卷积后的数据结构
                size_input_first_fc[0] = conv_kneral[0]
                ...
                conv_kneral = None
                if 'a' in self.size_layer[i]:
                    conv_kneral = np.array([16,8,3,3,3])
                else:
                ...
                # 添加侧链结构
                self.layers_pool.pop()
                self.layers_pool.append(LayerMapping.LayerMapping('map' + self.size_layer[i][-1]))
                self.layers_pool.append(LayerMaxPool.LayerMaxPool(size_kernel, step, self.size_layer[i]))
                ...
                if i == self.size_layer.__len__() - 1:
                    ...
                    fun = 'softmax'
                # 倒数第二层用sigmoid
                elif i == self.size_layer.__len__() - 2:
                    fun = 'sigmoid'
                # 其他用
                else:
                    fun = 'leaky_relu'
    ...
    def train(self, train_data_path_list, train_target, test_data_path_list, test_target, batch_size = 1, epoch = 1, mu = 0.05, path = ""):
        ...
        for i in range(epoch + 1):
            if i != 0:
                ...
                # 保存权重
                if rate > acc_rate_last_test:
                    print("保存权重中...")
                    self.save_par(path)
                    acc_rate_last_test = rate
                print("最高的正确率为：", (acc_rate_last_test * 100).__round__(2), "%")
                # 保存每个epoch过后的正确率
...
#
    ...
    def think(self, input_data):
        ...
        for i in range(0, self.layers.__len__()):
            ...
            if 'fc' not in self.layers[i].name:
                # 默认输入层的上一层为池化层
                if i == 0:
                    # 主干网络
                    ...
                    # 再算次干网络
                    if 'pool' in self.layers[i].name:

                        # 次干网络的第一个输入
                        if '1' in self.layers_pool[i].name:
                            # 调整输入数据的尺寸
                            mix_data = FeatureMix.mix(np.zeros(input_data.shape), input_data, self.layers[i - 1].a.shape)
                            # mapping 层
                            result = self.layers_pool[i - 1].think(mix_data, None)
                            # 池化层
                            result = self.layers_pool[i].think(self.layers_pool[i - 1].a, self.layers[i - 1].name)

                        else:
                            # 找到上一个池化层的位置
                            num = i - 1
                            for k in range(i - 1, -1, -1):
                                if self.layers_pool[k] is None or 'pool' not in self.layers_pool[k].name:
                                    num -= 1
                                else:
                                    break
                            # 调整输入数据的尺寸
                            mix_data = FeatureMix.mix(self.layers[num].a, self.layers_pool[num].a, self.layers[i - 1].a.shape)
                            # map 层
                            result = self.layers_pool[i - 1].think(mix_data, None)
                            # 池化层
                            result = self.layers_pool[i].think(self.layers_pool[i - 1].a, self.layers[i - 1].name)
        ...
        for i in range(self.layers.__len__() - 1, - 1, -1):
            ...
            if i == self.layers.__len__() - 1:
                ...
                if 'pool' in self.layers[i].name:
                    # 确定下一个map层的位置
                    num = i + 1
                    for k in range(i + 1, self.layers.__len__()):
                        if self.layers_pool[k] is None or 'map' not in self.layers_pool[k].name:
                            num += 1
                        else:
                            break

                    # 主干网络
                    result = self.layers[i].cal_delta_2(self.layers_pool[num].weight, None, None, self.layers_pool[num].delta, None, None)
                    # 次干网络
                    result = self.layers_pool[i].cal_delta_2(self.layers_pool[num].weight, None, None, self.layers_pool[num].delta, None, None)
                    result = self.layers_pool[i - 1].cal_delta(self.layers_pool[i].weight, self.layers_pool[i].step, None, self.layers_pool[i].delta, self.layers_pool[i].name, self.layers_pool[i].pool_position)
    ...
    def adjust_par(self, train_data, mu):
        ...
        for i in range(self.layers.__len__()):
            ...
            if 'pool' not in self.layers[i].name:
                ...
                elif self.layers[i].name == 'fc1a':
                    self.layers[i].adjust_par(self.layers[i - 1].a + self.layers_pool[i - 1].a, self.layers[i - 1].name, mu)
                else:
            ...
            if self.layers_pool[i] is not None and 'map' in self.layers_pool[i].name:
                self.layers_pool[i].adjust_par(None, None, mu)
