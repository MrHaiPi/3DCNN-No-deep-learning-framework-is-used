...
class LayerMapping:
    ...
    def think(self, pre_layer_a, pre_layer_name):
        ...
        self.z = np.zeros((pre_layer_a.shape[0],
                           pre_layer_a.shape[1] * self.weight.shape[0],
                           pre_layer_a.shape[2],
                           pre_layer_a.shape[3],
                           pre_layer_a.shape[4]))

        for i in range(self.z.shape[1]):
            self.z[:, i, :, :, :] = pre_layer_a[:, i // self.weight.shape[0], :, :, :] * self.weight[i % self.weight.shape[0]]
    ...
    def adjust_par(self, pre_layer_a, pre_layer_name, mu):
        ...
        for i in range(pre_layer_a.shape[0]):
            for j in range(self.weight.shape[0]):
                # 对每个通道进行计算
                for k in range(pre_layer_a.shape[1]):
                    w_delta[j] += 1 / self.z.shape[0] * (self.delta[i, k * self.weight.shape[0] + j] * pre_layer_a[i, k]).sum()
