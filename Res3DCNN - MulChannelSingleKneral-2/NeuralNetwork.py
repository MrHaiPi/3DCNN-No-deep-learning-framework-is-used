    ...
    def build_net(self, size_input_data, size_output_data, size_layer):
        ...
        for i in range(self.size_layer.__len__()):
            ...
            if 'conv1' in self.size_layer[i]:
                conv_kneral = np.array([2,3,3,3])#8
                ...
                conv_kneral = np.array([2,3,3,3])#16
                self.layers.append(LayerConv.LayerConv(conv_kneral, np.array([1, 1, 1]), np.array([1, 1, 1]), 'relu', self.size_layer[i]))
                self.layers_pool.append(LayerMapping.LayerMapping(np.array([conv_kneral[0], 1]), 'map' + self.size_layer[i][-2:]))
                # 更新卷积后的数据结构
                size_input_first_fc[0] = conv_kneral[0]
            elif 'conv3' in self.size_layer[i]:
                conv_kneral = np.array([16,3,3,3])#32
                self.layers.append(LayerConv.LayerConv(conv_kneral, np.array([1, 1, 1]), np.array([1, 1, 1]), 'relu', self.size_layer[i]))
                self.layers_pool.append(LayerMapping.LayerMapping(np.array([conv_kneral[0], 1]), 'map' + self.size_layer[i][-2:]))
                # 更新卷积后的数据结构
                size_input_first_fc[0] = conv_kneral[0]
            elif 'conv4' in self.size_layer[i]:
                conv_kneral = np.array([16,3,3,3])#64
                self.layers.append(LayerConv.LayerConv(conv_kneral, np.array([1, 1, 1]), np.array([1, 1, 1]), 'relu', self.size_layer[i]))
                self.layers_pool.append(LayerMapping.LayerMapping(np.array([conv_kneral[0], 1]), 'map' + self.size_layer[i][-2:]))
                # 更新卷积后的数据结构
                size_input_first_fc[0] = conv_kneral[0]
            elif 'conv5' in self.size_layer[i]:
                conv_kneral = np.array([32,3,3,3])#64
                self.layers.append(LayerConv.LayerConv(conv_kneral, np.array([1, 1, 1]), np.array([1, 1, 1]), 'relu', self.size_layer[i]))
                self.layers_pool.append(LayerMapping.LayerMapping(np.array([conv_kneral[0], 1]), 'map' + self.size_layer[i][-2:]))
                # 更新卷积后的数据结构
                size_input_first_fc[0] = conv_kneral[0]
            elif 'conv6' in self.size_layer[i]:
                conv_kneral = np.array([16,3,3,3])#4
                self.layers.append(LayerConv.LayerConv(conv_kneral, np.array([1, 1, 1]), np.array([1, 1, 1]), 'relu', self.size_layer[i]))
                self.layers_pool.append(LayerMapping.LayerMapping(np.array([conv_kneral[0], 1]), 'map' + self.size_layer[i][-2:]))
                # 更新卷积后的数据结构
                size_input_first_fc[0] = conv_kneral[0]
                ...
                if 'a' in self.size_layer[i]:
                    # 池化层的所有输出全连接至全连接层
                    num_pre = reduce(lambda x, y : x * y, size_input_first_fc)
                    num_cur = num_fc_neral
                else:
                    num_pre = num_fc_neral
                    num_cur = num_fc_neral

                # 倒数第一层用softmax
                if i == self.size_layer.__len__() - 1:
                    num_cur = self.size_output_data[0]
                    fun = 'softmax'
                # 其他用
                else:
                    fun = 'relu'
                self.layers.append(LayerFC.LayerFC(num_pre, num_cur, fun, self.size_layer[i]))
                self.layers_pool.append(None)
    ...
    def think(self, input_data):
        ...
        for i in range(0, self.layers.__len__()):
            ...
            if i == 0:
                ...
                result = self.layers_pool[i].think(input_data, 'pool')
            else:
                if 'conv' in self.layers[i].name:
                    if 'conv' in self.layers[i - 1].name:
                        result = self.layers[i].think(self.layers[i - 1].a, self.layers[i - 1].name)
                        result = self.layers_pool[i].think(self.layers_pool[i - 1].a, self.layers_pool[i - 1].name)
                    else:
                        result = self.layers[i].think(self.layers[i - 1].a, self.layers[i - 1].name)
                        result = self.layers_pool[i].think(self.layers[i - 1].a, self.layers[i - 1].name)
    ...
    def cal_delta(self, think_data, target_data):
        ...
        for i in range(self.layers.__len__() - 1, - 1, -1):
            ...
            if i == self.layers.__len__() - 1:
                ...
                if 'conv' in self.layers[i].name:
                    result = self.layers[i].cal_delta(next_layer_weight, next_layer_step, next_layer_padding, next_layer_delta, next_layer_name, next_layer_pool_position)
                    result = self.layers_pool[i].cal_delta(self.layers_pool[i + 1].weight, None, None, self.layers_pool[i + 1].delta, self.layers_pool[i + 1].name, None)
                else:
                    result = self.layers[i].cal_delta(next_layer_weight, next_layer_step, next_layer_padding, next_layer_delta, next_layer_name, next_layer_pool_position)
        ...
        for i in range(self.layers.__len__()):
            ...
            if 'pool' not in self.layers[i].name:
                ...
                if i == 0:
                    ...
                    if 'conv' in self.layers[i].name:
                        if 'conv' in self.layers[i - 1].name:
                            result = self.layers[i].adjust_par(self.layers[i - 1].a, self.layers[i - 1].name, mu)
                            result = self.layers_pool[i].adjust_par(self.layers_pool[i - 1].a, self.layers_pool[i - 1].name, mu)
                        else:
                            result = self.layers[i].adjust_par(self.layers[i - 1].a, self.layers[i - 1].name, mu)
                            result = self.layers_pool[i].adjust_par(self.layers[i - 1].a, self.layers[i - 1].name, mu)
                    else:
                        result = self.layers[i].adjust_par(self.layers[i - 1].a, self.layers[i - 1].name, mu)
